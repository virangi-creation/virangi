...
function InputCatalogue() {
    ...
    const onSubmitEvent = async () => {
        try {
            if (catalogueNo && designNo && manufacturerPrice && sellPrice) {
                setLoad(true);
                await axios
                    .post("/catalogue/", {
                        catalogueNo,
    ...
    const captureEnter = async (event) => {
        ...
        if (event.keyCode === 27) {
            const selection = await window.confirm(
    ...
    useEffect(() => {
        setManufacturerPrice(maxManufacturerPrice);
    }, [maxManufacturerPrice]);

    useEffect(() => {
        setSellPrice(maxSellPrice);
    }, [maxSellPrice]);
